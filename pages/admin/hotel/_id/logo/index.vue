<template>
  <div>
    ...
    <v-row align="center" justify="space-between" no-gutters>
      <v-col cols="auto">
        <v-card-title class="black--text pa-2">{{$t('Logo')}}</v-card-title>
      </v-col>
      <v-col cols="auto">
    ...
    <v-card-text class="pa-2">
      <v-card
        ...
        class="rounded"
      >
        <v-img class="secondary" :src="mainUrl" :aspect-ratio="1" />
      </v-card>
    ...
    async submit(image) {
      ...
      try {
        await this.$axios.post(
          (LOGO.replace("__ENGINE_ID__", this.$store.state.info.id)),
          formData,
          {
